...
app.use(cors()); // 1. لازم اول شي
app.use(express.json()); // 2. ثاني شي

const readTasks = () => {
  if (!fs.existsSync(DATA_FILE)) return [];
  return JSON.parse(fs.readFileSync(DATA_FILE));
};
const saveTasks = (tasks) => fs.writeFileSync(DATA_FILE, JSON.stringify(tasks));

// 3. كل الـ Routes تجي بعد cors
